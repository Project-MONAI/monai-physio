...
class TestToolsForDataDownloads:
    ...
    def test_verify_slicer_heart_ct_data(self, tmp_path: Path) -> None:
        ...
        data_file = tmp_path / ToolsForDataDownloads.SLICER_HEART_CT_FILENAME

        assert not ToolsForDataDownloads.VerifySlicerHeartCTData(tmp_path)

        data_file.write_bytes(b"nrrd")

        assert ToolsForDataDownloads.VerifySlicerHeartCTData(tmp_path)
    ...
    def test_verify_kcl_heart_model_data(self, tmp_path: Path) -> None:
        ...
        assert not ToolsForDataDownloads.VerifyKCLHeartModelData(tmp_path)

        (input_meshes / "01.vtk").write_text("# vtk\n")

        assert ToolsForDataDownloads.VerifyKCLHeartModelData(tmp_path)
    ...
    def test_verify_dirlab_4dct_data(self, tmp_path: Path) -> None:
        ...
        assert not ToolsForDataDownloads.VerifyDirLab4DCTData(tmp_path)
        ...
        assert ToolsForDataDownloads.VerifyDirLab4DCTData(tmp_path)
    ...
    def test_verify_chop_valve_4d_data(self, tmp_path: Path) -> None:
        ...
        assert not ToolsForDataDownloads.VerifyCHOPValve4DData(tmp_path)

        ct_dir = tmp_path / "CT"
        ct_dir.mkdir()
        (ct_dir / "RVOT28-Dias.nii.gz").write_bytes(b"nii")

        assert ToolsForDataDownloads.VerifyCHOPValve4DData(tmp_path)
...
class TestDownloadHeartData:
    ...
    def test_data_downloaded(
        ...
        assert ToolsForDataDownloads.VerifySlicerHeartCTData(
    ...
    def test_download_kcl_heart_model_data(
        ...
        urls_to_archives[ToolsForDataDownloads.KCL_HEART_MODEL_AVERAGE_URL] = (
            make_archive("average.vtk", b"# vtk average\n")
        )
        ...
        monkeypatch.setattr(
            "monai_physio.tools_for_data_downloads.urllib.request.urlopen", fake_urlopen
        )

        output_dir = tmp_path / "KCL-Heart-Model"
        result_dir = ToolsForDataDownloads.DownloadKCLHeartModelData(output_dir)
        ...
        assert ToolsForDataDownloads.VerifyKCLHeartModelData(output_dir)
    ...
    def test_download_chop_valve4d_data(
        ...
        result_dir = ToolsForDataDownloads.DownloadCHOPValve4DData(output_dir)

        assert result_dir == output_dir
        for subdir_name in ToolsForDataDownloads.CHOP_VALVE4D_ASSETS:
    ...
    def test_download_chop_valve4d_data_redownloads_partial_extraction(
        ...
        ToolsForDataDownloads.DownloadCHOPValve4DData(output_dir)
