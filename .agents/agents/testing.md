...
- `src/monai_physio/tools_for_tests.py` - baseline comparison utilities (`ToolsForTests`)
...
python -m pytest tests/test_contour_tools.py::TestToolsForContours -v # single class
...
6. When a test produces an image or surface, compare against a baseline using
   `tools_for_tests.py` utilities (`ToolsForTests`) rather than ad-hoc value asserts.
