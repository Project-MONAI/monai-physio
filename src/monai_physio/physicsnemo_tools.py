...
``torch`` and ``torch_geometric`` are base dependencies, but every method
...
class PhysicsNemoTools:
    ...
    is a grouping rather than an object to construct. Wrapping does not change
    when a method's own local ``import torch``/``import torch_geometric`` runs
    - only at call time - so ``import monai_physio`` stays as lightweight as
    it was when these were bare functions.
    """
