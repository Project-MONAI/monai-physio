...
:class:`monai_physio.TrainPhysicsNeMoMGN` scores predicted motion against
...
monai_physio`` works without it.  It ships inside ``nvidia-physicsnemo``.
"""
...
class NeoHookeanResidual(MONAIPhysioBase):
...
class PhysicsInformedMotion(MONAIPhysioBase):
...
class TrainPhysicsNeMoPhysicsInformedMotion(TrainPhysicsNeMoMGN):
    ...
    Identical to :class:`monai_physio.TrainPhysicsNeMoMGN` apart from the loss,
