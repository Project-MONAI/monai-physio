...
* ``ToolsForTransforms`` applies transforms to images and PyVista contours.
* ``ToolsForContours`` creates and transforms VTK/PyVista surface data.
* ``ToolsForUSD`` and ``ToolsForUSDAnatomy`` operate on USD stages and files.
