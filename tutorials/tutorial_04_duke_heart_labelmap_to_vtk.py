...
- ``<frame_stem>_surfaces.vtp`` -- every label's watertight, outward-oriented
  surface in one file, via ``ToolsForContours.extract_label_surfaces`` on the whole
...
- ``<frame_stem>_<name>.vtu`` -- one tetrahedral mesh per structure, six
  ``VTK_TETRA`` per isotropic voxel, via ``ToolsForContours.extract_tetrahedra``,
  then relaxed onto that structure's surface by
  ``ToolsForContours.trim_tetrahedra_to_surface``.
...
Both carry the structure's ``ToolsForUSDAnatomy`` color, as ``AnatomyColor`` and
...
if __name__ == "__main__":
    ...
    def annotate(
        ...
        ``ToolsForContours.save_combined_surfaces`` needs to tag the merged file's
        cells with the structure they came from.  The anatomy color is attached
        separately, by ``ToolsForContours.apply_anatomy_color``.
        """
    ...
    def write_tetrahedra(
        ...
        while True:
            # ToolsForUSDAnatomy has no override for names like "left_ventricle",
