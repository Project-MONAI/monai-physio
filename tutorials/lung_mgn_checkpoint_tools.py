...
from monai_physio import ProcessTransforms, WorkflowFitStatisticalModelToPatient
...
def prepare_checkpoint_compatible_lung_case(
    ...
    ----------
    source_case_dir
        Tutorial 8 output directory for one TCIA-4DLung case.
    model_dir
    ...
    transform_files = sorted(
        source_case_dir.glob(f"{case_id}_g0??_forward_tfm.hdf")
    )
